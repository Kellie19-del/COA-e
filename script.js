...
async function loadData() {
    try {
        ...
        toast(`Database unavailable: ${err.message}. Using fallback session.`, 'error');
    }
}
...
function commit() {
    commitQueue = commitQueue
        ...
        .catch((err) => {
            ...
            toast(`Data could not be saved: ${err.message}`, 'error');
        });
...
function setupEventListeners() {
    ...
    DOM.dashboard.navItems.forEach(item => {
        item.onclick = (e) => {
            ...
            DOM.dashboard.navItems.forEach(nav => nav.classList.remove('active'));
            DOM.dashboard.navItems.forEach(nav => {
...
function setupUploadInteractions() {
    ...
    ['dragenter', 'dragover'].forEach(evt => {
        zone.addEventListener(evt, (e) => {
            e.preventDefault();
            zone.classList.add('border-blue-900', 'bg-blue-50');
        });
    });
    ...
    ['dragleave', 'drop'].forEach(evt => {
        zone.addEventListener(evt, (e) => {
            e.preventDefault();
            zone.classList.remove('border-blue-900', 'bg-blue-50');
        });
    });
...
function renderReports() {
    ...
    DOM.data.reportsBody.innerHTML = data.length ? data.map(r => `
        <tr class="border-b border-slate-100">
            <td data-label="ID Ref" class="px-2 py-3"><strong>#${r.id}</strong></td>
            <td data-label="Unit" class="px-2 py-3">${escapeHtml(r.unit)}</td>
            <td data-label="Deadline" class="px-2 py-3">${escapeHtml(r.dueDate)}</td>
            <td data-label="Status" class="px-2 py-3">
                <span class="${r.status === 'completed'
            ? 'inline-flex rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold uppercase text-emerald-700'
            : 'inline-flex rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold uppercase text-amber-700'}">${escapeHtml(r.status)}</span>
            </td>
            <td data-label="Docs">
                ${r.file && r.fileData
            ? `<button class="inline-flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" onclick="openDocument('${r.id}')"><i class="ri-eye-line"></i> ${escapeHtml(r.file)}</button>`
            : r.file
                ? `<span class="text-slate-600"><i class="ri-file-line"></i> ${escapeHtml(r.file)}</span>`
                : '<span class="text-slate-400">-</span>'}
            </td>
            <td data-label="Actions">
                <div class="flex items-center gap-2">
                    ${role === 'unit' && r.status === 'pending'
            ? `<button class="inline-flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" onclick="openUpload('${r.id}')"><i class="ri-upload-2-line"></i> Upload</button>`
            : ''}
                    ${role === 'admin'
            ? `<button class="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs font-semibold text-red-700 hover:bg-red-100" onclick="deleteReport(${r.id})"><i class="ri-delete-bin-line"></i></button>`
            : ''}
                    ${r.status === 'completed' && role === 'unit' ? '<button class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-500" disabled><i class="ri-check-line"></i> Done</button>' : ''}
                </div>
            </td>
        </tr>
    `).join('') : '<tr><td colspan="6" class="px-2 py-6 text-center text-slate-400">No records found</td></tr>';
}
    ...
    DOM.data.inboxList.innerHTML = myMsgs.length ? myMsgs.map(m => `
        <div class="rounded-xl border border-slate-200 bg-white p-3">
            <div class="flex gap-3">
                <i class="ri-notification-3-line text-red-700"></i>
                <div>
                    <h4 class="font-semibold text-slate-900">${role === 'admin' ? `Notice to ${escapeHtml(m.to)}` : 'Admin Notice'}</h4>
                    <p class="text-sm text-slate-700">${escapeHtml(m.msg)}</p>
                    <span class="text-xs text-slate-500">${escapeHtml(m.time)}</span>
                </div>
            </div>
        </div>
    `).join('') : '<div class="px-5 py-6 text-center text-slate-400">All caught up!</div>';
}
...
function renderAudit() {
    ...
    DOM.data.auditList.innerHTML = store.audit.map(a => `
        <div class="rounded-xl border border-slate-200 bg-white p-3">
            <span class="block text-sm font-semibold text-slate-900">${escapeHtml(a.actor)}: ${escapeHtml(a.action)}</span>
            <span class="text-xs text-slate-500">${escapeHtml(a.timestamp)}</span>
        </div>
    `).join('') || '<div class="px-5 py-6 text-center text-slate-400">No logs yet.</div>';
}
...
function renderUnits() {
    ...
    const units = store.users.map(u => `
        <tr class="border-b border-slate-100">
            <td data-label="Unit Name" class="px-2 py-3"><strong>${escapeHtml(u.username)}</strong></td>
            <td data-label="Role" class="px-2 py-3"><span class="${u.role === 'admin' ? 'inline-flex rounded-full bg-red-700 px-2 py-1 text-xs font-semibold text-white' : 'inline-flex rounded-full bg-blue-950 px-2 py-1 text-xs font-semibold text-white'}">${u.role.toUpperCase()}</span></td>
            <td data-label="Status" class="px-2 py-3"><span class="inline-flex rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold uppercase text-emerald-700">Active</span></td>
            <td data-label="Actions">
                ${u.username === 'kellie' ? '<span class="text-slate-400">System Owner</span>' : `
                    <div class="flex items-center gap-2">
                        <button class="inline-flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50" onclick="openUnitModal(decodeURIComponent('${encodeURIComponent(u.username)}'))">
                            <i class="ri-edit-line"></i> Edit
                        </button>
                        <button class="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-2 py-1 text-xs font-semibold text-red-700 hover:bg-red-100" onclick="deleteUser(decodeURIComponent('${encodeURIComponent(u.username)}'))">
    ...
    tbody.innerHTML = units || '<tr><td colspan="4" class="px-2 py-6 text-center text-slate-400">No units found</td></tr>';
}
...
window.openDocument = (id) => {
    ...
    if (ext === 'pdf') {
        DOM.docModal.content.innerHTML = `<iframe class="h-[60vh] w-full rounded-lg border border-slate-200" src="${report.fileData}" title="${escapeHtml(report.file)}"></iframe>`;
    } else if (ext === 'csv') {
        const rawCsv = decodeDataUrlText(report.fileData);
        DOM.docModal.content.innerHTML = `<textarea class="min-h-[320px] w-full rounded-lg border border-slate-200 p-3 font-mono text-xs" readonly></textarea>`;
        ...
        DOM.docModal.content.innerHTML = `
            <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                <p class="mt-2">Use the Download button to open this file on your device.</p>
            </div>
        `;
    }
...
function toast(msg, type = 'success') {
    ...
    el.className = `rounded-xl px-3 py-2 text-sm font-medium text-white shadow-xl ${type === 'success' ? 'bg-blue-950' : 'bg-red-700'}`;
